function updateUI() {
	...
	$("autocapamt").innerText = player.upgrades.includes("24") ? "无软上限" : `软上限为每个自动书写器写 ${toSci(player.upgrades.includes("13") ? player.auto.mul((player.upgrades.includes("22") ? 4 : 2)).add(1) : new Decimal(1).mul(1e5))} 个汉字`;
	$("bulkautoamt").innerText = `${1} 个自动书写器`;
